...
class ListingModel extends Model
{
    ...
    {
        $bestBidSql = '(SELECT MAX(user_bid.montant) FROM `ENCHERE` user_bid'
            . ' WHERE user_bid.annonce_id = listing.id AND user_bid.utilisateur_id = :best_user_id)';
        ...
        $sql = 'SELECT listing.id, listing.titre, listing.etat_objet, listing.prix_depart,'
            . ' listing.date_heure_fin, listing.categorie_libelle,'
            . ' (SELECT MAX(all_bid.montant) FROM `ENCHERE` all_bid'
            . ' WHERE all_bid.annonce_id = listing.id) AS best_bid,'
            . ' (SELECT COUNT(*) FROM `ENCHERE` counted_bid'
            . ' WHERE counted_bid.annonce_id = listing.id) AS bid_count,'
            . ' ' . $bestBidSql . ' AS user_best_bid,'
            . ' ' . $winnerSql . ' AS winner_id,'
            . ' EXISTS(SELECT 1 FROM `ASSOC_UTILISATEUR_ANNONCE` followed'
            . ' WHERE followed.annonce_id = listing.id AND followed.utilisateur_id = :follow_user_id)'
            . ' AS is_following'
            . ' FROM `ANNONCE` listing'
            . ' WHERE listing.utilisateur_id <> :owner_user_id'
            . ' AND ((listing.date_heure_fin > UTC_TIMESTAMP() AND ('
            . ' EXISTS(SELECT 1 FROM `ASSOC_UTILISATEUR_ANNONCE` active_follow'
            . ' WHERE active_follow.annonce_id = listing.id'
            . ' AND active_follow.utilisateur_id = :active_follow_user_id)'
            . ' OR EXISTS(SELECT 1 FROM `ENCHERE` active_bid'
            . ' WHERE active_bid.annonce_id = listing.id'
            . ' AND active_bid.utilisateur_id = :active_bid_user_id)))'
            . ' OR (listing.date_heure_fin <= UTC_TIMESTAMP()'
            . ' AND EXISTS(SELECT 1 FROM `ENCHERE` ended_bid'
            . ' WHERE ended_bid.annonce_id = listing.id'
            . ' AND ended_bid.utilisateur_id = :ended_bid_user_id)))'
            . ' ORDER BY CASE WHEN listing.date_heure_fin > UTC_TIMESTAMP() THEN 0 ELSE 1 END,'
            . ' listing.date_heure_fin ASC, listing.id ASC';
