...
if ($categoriesUnavailable) {
    ...
    $eyebrow = 'GESTION DE L’ANNONCE';
} elseif (isset($errors['form']) && array_diff(array_keys($errors), ['form']) === []) {
        ...
        <section class="listing-form-panel glass-panel" aria-labelledby="listing-form-title">
            ...
            <form action="index.php?route=<?= $formRoute ?>" method="post" enctype="multipart/form-data" novalidate data-listing-form>
                ...
                <div class="form-grid listing-form-grid">
                    <div class="form-field listing-field--title"><label class="form-field__label" for="title">Titre *</label><input class="form-control" id="title" name="title" type="text" required minlength="3" maxlength="255" placeholder="Titre de l’objet" value="<?= htmlspecialchars((string) $values['title'], ENT_QUOTES, 'UTF-8') ?>" <?= isset($errors['title']) ? 'aria-invalid="true"' : '' ?> <?= $lockedAttribute ?>><span class="form-field__error"><?= isset($errors['title']) ? htmlspecialchars((string) $errors['title'], ENT_QUOTES, 'UTF-8') : '' ?></span></div>
                    <div class="form-field listing-field--category"><label class="form-field__label" for="category">Catégorie *</label><select class="form-control" id="category" name="category" required <?= $categoriesUnavailable ? 'disabled' : '' ?> <?= isset($errors['category']) ? 'aria-invalid="true"' : '' ?> <?= $lockedAttribute ?>><option value=""><?php if ($categoriesUnavailable): ?>Indisponible<?php else: ?>Choisir une catégorie<?php endif; ?></option><?php foreach ($categories as $identifier => $label): ?><option value="<?= (int) $identifier ?>" <?= (string) $values['category'] === (string) $identifier ? 'selected' : '' ?>><?= htmlspecialchars($label, ENT_QUOTES, 'UTF-8') ?></option><?php endforeach; ?></select><span class="form-field__error"><?php if ($categoriesUnavailable): ?>Catégories temporairement indisponibles.<?php endif; ?></span></div>
                    <div class="form-field listing-field--description"><label class="form-field__label" for="description">Description *</label><textarea class="form-control" id="description" name="description" required minlength="10" maxlength="5000" placeholder="Décrivez précisément l’objet proposé." <?= isset($errors['description']) ? 'aria-invalid="true"' : '' ?> <?= $lockedAttribute ?>><?= htmlspecialchars((string) $values['description'], ENT_QUOTES, 'UTF-8') ?></textarea><span class="form-field__error"><?= isset($errors['description']) ? htmlspecialchars((string) $errors['description'], ENT_QUOTES, 'UTF-8') : '' ?></span></div>
                    <div class="form-field listing-field--state"><label class="form-field__label" for="item-state">État de l’objet *</label><select class="form-control" id="item-state" name="item_state" required <?= isset($errors['item_state']) ? 'aria-invalid="true"' : '' ?> <?= $lockedAttribute ?>><option value="">Choisir un état</option><?php foreach (['neuf', 'très bon état', 'bon état', 'état correct'] as $state): ?><option value="<?= htmlspecialchars($state, ENT_QUOTES, 'UTF-8') ?>" <?= $values['item_state'] === $state ? 'selected' : '' ?>><?= htmlspecialchars(ucfirst($state), ENT_QUOTES, 'UTF-8') ?></option><?php endforeach; ?></select><span class="form-field__error"></span></div>
                    <div class="form-field listing-field--price"><label class="form-field__label" for="starting-price">Prix de départ *</label><input class="form-control" id="starting-price" name="starting_price" type="number" required min="0.01" step="0.01" placeholder="0,00 €" value="<?= htmlspecialchars((string) $values['starting_price'], ENT_QUOTES, 'UTF-8') ?>" <?= $lockedAttribute ?>><span class="form-field__error"><?= isset($errors['starting_price']) ? htmlspecialchars((string) $errors['starting_price'], ENT_QUOTES, 'UTF-8') : '' ?></span></div>
                    <div class="form-field listing-field--date"><label class="form-field__label" for="end-date">Date de fin *</label><input class="form-control" id="end-date" name="end_date" type="date" required placeholder="JJ/MM/AAAA" value="<?= htmlspecialchars((string) $values['end_date'], ENT_QUOTES, 'UTF-8') ?>" <?= $lockedAttribute ?>><span class="form-field__error"><?= isset($errors['end_date']) ? htmlspecialchars((string) $errors['end_date'], ENT_QUOTES, 'UTF-8') : '' ?></span></div>
                    <div class="form-field listing-field--time"><label class="form-field__label" for="end-time">Heure de fin *</label><input class="form-control" id="end-time" name="end_time" type="time" required placeholder="HH:MM" value="<?= htmlspecialchars((string) $values['end_time'], ENT_QUOTES, 'UTF-8') ?>" <?= $lockedAttribute ?>><span class="form-field__help">Heure de Paris — Europe/Paris</span><span class="form-field__error"><?php if (isset($errors['end_time'])): ?><?= htmlspecialchars((string) $errors['end_time'], ENT_QUOTES, 'UTF-8') ?><?php elseif (isset($errors['deadline'])): ?><?= htmlspecialchars((string) $errors['deadline'], ENT_QUOTES, 'UTF-8') ?><?php endif; ?></span></div>
                </div>

                <fieldset class="photo-manager" data-existing-photo-count="<?= count($existingPhotos) ?>" <?= $lockedAttribute ?>><legend>Photographies</legend><p class="photo-manager__help">3 photographies maximum · la première ajoutée est l’image principale · toute nouvelle photo est ajoutée à la fin.</p><span class="photo-manager__counter" data-photo-counter><?= count($existingPhotos) ?> / 3</span>
                ...
                <div class="listing-form-actions">
                    ...
                    <?php else: ?>
                        ...
                        <p><?php if ($categoriesUnavailable): ?>Aucune catégorie locale de remplacement n’est proposée.<?php elseif ($isEditMode): ?>Modification possible tant qu’aucune enchère n’est enregistrée et avant l’échéance.<?php else: ?>Le prix de départ doit être strictement positif et comporter au maximum deux décimales.<?php endif; ?></p>
                    <?php endif; ?>
                </div>
            </form>
