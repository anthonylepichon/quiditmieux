            ...
            <div class="listing-summary glass-panel">
                ...
                <?php if ($viewer['can_bid']): ?>
                    <form class="bid-form" action="index.php?route=place_bid" method="post" data-bid-form>
                        ...
                        <div class="bid-form__row"><input class="form-control" id="bid-amount" name="amount" type="number" min="<?= htmlspecialchars($listing['minimum_bid'], ENT_QUOTES, 'UTF-8') ?>" step="0.01" placeholder="<?= htmlspecialchars($listing['minimum_bid_label'], ENT_QUOTES, 'UTF-8') ?> minimum"<?php if ($bidRejection !== null): ?> value="<?= htmlspecialchars($bidRejection['amount'], ENT_QUOTES, 'UTF-8') ?>"<?php endif; ?> required><button class="button button--primary" type="submit">Enchérir</button></div>
                        <p class="form-field__help" data-minimum-bid><?php if ($bidRejection !== null): ?>Montant insuffisant : minimum <?= htmlspecialchars($bidRejection['minimum_label'], ENT_QUOTES, 'UTF-8') ?>.<?php else: ?>Montant supérieur d’au moins 0,01 € au prix courant.<?php endif; ?></p>
